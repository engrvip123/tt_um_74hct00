...
2-input NAND** logic IC, realized in **gf180mcuD** PDK at **3.3 V**.

* [Read information about this project](docs/info.md)

## Why this design

The 74HCT00 family is a TTL-compatible CMOS NAND part that
boards have used for long to translate TTL-level signals into
CMOS logic (V\_IL\_max = 0.8 V, V\_IH\_min = 2.0 V). Addressing the obsolescence
...
not on the digital `uo\_out` bus, so the bench can measure VOH/VOL at
...
the HCT input window on silicon. Driving A1 through a digital `ui\_in`
...
(B1, A2 / B2, A3 / B3, A4 / B4) come in on `ui\_in\[1..7]`; `ui\_in\[0]`
is left unused.

See [docs/info.md](docs/info.md) for the full pin table and bench measurement procedure.
